...
class Builder:
  ...
  def start(self):
      ...
      # Build Boost
      print '-- Building Boost - check spm_build.log for progress - estimated time 30 minutes'
      os.chdir(boostFileName)
  
      os.system('./bootstrap.sh gcc 1> spm_bootstrap.log 2>&1')
      os.system("./b2 address-model=64 architecture=x86 --toolset=gcc link=static threading=multi -j " + Globals.threads + " cxxflags='-fPIC' 1> spm_build.log 2>&1")
