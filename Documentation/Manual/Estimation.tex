...
\begin{enumerate}
\item You can request that the stepsize change adaptively at one or more sample numbers. At each adaptation, the stepsize is modified using the acceptance rate, by increasing or decreasing the stepsize by the ratio of the difference between the current acceptance rate (since the previous adaption) and the target acceptance rate. The default acceptance ratio is $0.24$ (See Gelman et al. \citep{823} for justification).
