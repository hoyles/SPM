  ...
  def start(self):
    ...
    if Globals.operating_system_ == "windows":
      if not os.path.exists(Globals.boost_source_directory_ + '/windows.py'):
        return Globals.PrintError('Boost library' + folder + ' does not have a windows.py file.\nThis file is required to build this library on Windows')
      ...
      if not os.path.exists(Globals.boost_directory_ + '/linux.py'):
        return Globals.PrintError('Boost library ' + Globals.boost_directory_ + ' does not have a linux.py file.\nThis file is required to build this library on Linux')
